...
class Led(Component):
    def __init__(self, pin_num, blink_dur=None, perc_brightness=100, on=False, debug_enabled=False):
        ...
        if blink_dur is not None:
            ...
            self.pwm = PWM(self.pin, freq=4000000, duty=0)
        self.emitting = False
...
class Photoresistor(Component):
    def __init__(self, pin_num, debug_enabled=False, measurement_interval=500, filename=None, on=False, measurement_duration=None):
        ...
        if measurement_duration:
            self.measurement_duration = measurement_duration
        else:
            self.measurement_duration = 10000000000
    ...
    def update_filenames(self):
        self.save_filename = self.filenames[0] + (f"_{self.filename_counter}")
    ...
    def check_stop(self, curr_time):
        if ticks_diff(curr_time, self.measurement_start_time)  >= self.measurement_duration:
